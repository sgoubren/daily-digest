...
from SSG_email import DailyDigestEmail  #you need to import the file related to email sending
from SSG_scheduler import DailyDigestScheduler #you need to import the file for scheduler
...
class DailyDigestGUI:
    ...
    def __init__(self, root):
        ...
        self.__style.configure('TButton', font = ('Arial', 12, 'bold'))  # all the buttons on the root will be 12
        self.__style.configure('Header.TLabel', font = ('Arial', 18, 'bold'))  # all the headers in the root will be 18

        # GUI listbox for recipients
        recipients_frame = ttk.Frame(self.__root)  # using the FRame class, that is inside the root window
        recipients_frame.pack(padx = 5, pady = 5)   # pack is a gemoetry manager, you put the widget and center it
        ...
        self.__top_news_var.set(self.__email.content['top_news']['include'])
    ...
    def __build_gui_contents(self, master, quote_var, weather_var, twitter_var, wikipedia_var):
        ...
        quote_checkbox = Checkbutton(master, text = 'Saying of the Day',
                                     onvalue = True, offvalue = False, 
                                     variable = quote_var)
        weather_checkbox = Checkbutton(master, text = 'Weather Forecast',
                                          onvalue = True, offvalue = False, 
                                          variable = weather_var)
        top_news_checkbox = Checkbutton(master, text = 'Top News',
                                           onvalue = True, offvalue = False,
        ...
        top_news_checkbox.grid(row = 1, column = 1, sticky = W)
    ...
    def __update_settings(self):
        ...
        self.__email.content['top_news']['include'] = self.__top_news_var.get()
